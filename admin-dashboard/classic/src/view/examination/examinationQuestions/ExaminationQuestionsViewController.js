Ext.define('Admin.view.examination.examinationQuestions.ExaminationQuestionsViewController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.examinationQuestionsViewController',
 /*******************************弹窗按钮***********************************/
         /*编辑修改弹窗按钮*/
	openExaminationQuestionsEditWindow:function(grid, rowIndex, colIndex){
		var record = grid.getStore().getAt(rowIndex);
		if (record ) {
			var win = grid.up('container').add(Ext.widget('examinationQuestionsEditWindow'));
	             win.show();	      
		      win.down('form').getForm().loadRecord(record);
	      }
	},
	/*多条件查询弹窗按钮*/	
	openExaminationQuestionsSearchWindow:function(toolbar, rowIndex, colIndex){
		toolbar.up('grid').up('container').add(Ext.widget('examinationQuestionsSearchWindow')).show();		
	},
	/*添加记录弹窗按钮*/	
	openExaminationQuestionsAddWindow:function(grid, rowIndex, colIndex){
		grid.up('container').up('gridpanel').add(Ext.widget('examinationQuestionsAddWindow')).show();
	},
 /*******************************点击按钮***********************************/
	/*分类单条件查询（快速查询）按钮*/
	quickSearch:function(button){
		...
		Ext.apply(store.proxy.extraParams, {state:"",category:"",createTimeStart:"",createTimeEnd:""});
		
		if(searchField==='state'){
			Ext.apply(store.proxy.extraParams, {state:searchFieldValue});
		}
		if(searchField==='category'){
			Ext.apply(store.proxy.extraParams, {category:searchFieldValue});
		}
  		...
  		if (selModel.hasSelection()) {
    			Ext.Msg.confirm('警告', '确定要删除吗？', function(btn) {
      			if (btn == 'yes') {
        				...
        				Ext.Ajax.request({url:'/examinationQuestions/deletes', method:'post', params:{ids:selectIds}, success:function(response, options) {
...
/*******************************提交按钮***********************************/
	/*编辑一条记录保存按钮*/
	examinationQuestionsEditSubmitButton:function(button){
		var win=button.up('window');
		var  store=Ext.data.StoreManager.lookup('examinationQuestionsStore')
	...
	/*添加一条记录提交按钮*/
	examinationQuestionsAddSubmitButton:function(button){
		var win=button.up('window')
		var form=win.down('form');
		var record = Ext.create('Admin.model.examinationQuestions.ExaminationQuestionsModel');
		var values  =form.getValues();//获取form数据 
			console.log(values);
           	record.set(values);
           	record.save();
           	Ext.data.StoreManager.lookup('examinationQuestionsStore').load();
             win.close();
	},
	/*查询多条记录条件提交按钮*/
	examinationQuestionsSearchSubmitButton:function(button){
		var store = Ext.data.StoreManager.lookup('examinationQuestionsStore');	
		var win=button.up('window');
		var form=win.down('form');

Ext.define('Admin.view.archives.ArchivesViewController', {
 ...
 /*******************************弹窗按钮***********************************/
	...
	openAddWindow:function(grid, rowIndex, colIndex){
		grid.up('container').add(Ext.widget('archivesAddWindow')).show();
	},
 /*******************************点击按钮***********************************/
	/*分类单条件查询（快速查询）按钮*/
  		...
  		if (selModel.hasSelection()) {
    			Ext.Msg.confirm('警告', '确定要删除吗？', function(btn) {
      			if (btn == 'yes') {
        				...
        				Ext.Ajax.request({url:'/archives/deletes', method:'post', params:{ids:selectIds}, success:function(response, options) {
...
/*******************************提交按钮***********************************/
	...
	editSubmitButton:function(button){
		...
		var  store=Ext.data.StoreManager.lookup('archivesGridStore')
	...
	addSubmitButton:function(button){
		...
		var values  =form.getValues();//获取form数据 
			console.log(values);
           	record.set(values);
           	record.save();
           	Ext.data.StoreManager.lookup('archivesGridStore').load();
             win.close();
	},
	/*查询多条记录条件提交按钮*/

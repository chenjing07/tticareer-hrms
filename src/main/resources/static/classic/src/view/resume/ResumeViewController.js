Ext.define('Admin.view.resume.ResumeViewController', {
 ...
 /*******************************弹窗按钮***********************************/
         /*编辑修改弹窗按钮*/
	openResumeEditWindow:function(grid, rowIndex, colIndex){
	...
	/*多条件查询弹窗按钮*/	
	openResumeSearchWindow:function(toolbar, rowIndex, colIndex){
		toolbar.up('grid').up('container').add(Ext.widget('resumeSearchWindow')).show();		
	},
	/*添加记录弹窗按钮*/	
	openResumeAddWindow:function(grid, rowIndex, colIndex){
		grid.up('container').up('gridpanel').add(Ext.widget('resumeAddWindow')).show();
	},
 /*******************************点击按钮***********************************/
	/*分类单条件查询（快速查询）按钮*/
	quickSearch:function(button){
		...
		Ext.apply(store.proxy.extraParams, {applicationName:"",state:"",createTimeStart:"",createTimeEnd:""});
		
		if(searchField==='applicationName'){
			Ext.apply(store.proxy.extraParams, {applicationName:searchFieldValue});
		}
  		...
  		if (selModel.hasSelection()) {
    			Ext.Msg.confirm('警告', '确定要删除吗？', function(btn) {
      			if (btn == 'yes') {
        				...
        				Ext.Ajax.request({url:'/resume/deletes', method:'post', params:{ids:selectIds}, success:function(response, options) {
...
/*******************************提交按钮***********************************/
	/*编辑一条记录保存按钮*/
	resumeEditSubmitButton:function(button){
	...
	/*添加一条记录提交按钮*/
	resumeAddSubmitButton:function(button){
		var win=button.up('window')
		...
		var values  =form.getValues();//获取form数据 
			console.log(values);
           	record.set(values);
           	record.save();
           	Ext.data.StoreManager.lookup('resumeStore').load();
             win.close();
	},
	/*查询多条记录条件提交按钮*/
	resumeSearchSubmitButton:function(button){
		var store = Ext.data.StoreManager.lookup('resumeStore');	
		var win=button.up('window');
		var form=win.down('form');
		var values = form.getValues();
		Ext.apply(store.proxy.extraParams, {applicationName:'',createTimeStart:'', createTimeEnd:''});
		Ext.apply(store.proxy.extraParams, {applicationName:values.applicationName,createTimeStart:Ext.util.Format.date(values.createTimeStart, 'Y/m/d H:i:s'), createTimeEnd:Ext.util.Format.date(values.createTimeEnd, 'Y/m/d H:i:s')});
		store.load({params:{start:0, limit:20, page:1}});
		win.close();
	}
});

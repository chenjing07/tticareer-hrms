Ext.define('Admin.view.expenseaccount.ExpenseAccountViewController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.expenseAccountViewController',
 /*******************************弹窗按钮***********************************/
         /*编辑修改弹窗按钮*/
	openEditWindow:function(grid, rowIndex, colIndex){
		var record = grid.getStore().getAt(rowIndex);
		if (record ) {
			var win = grid.up('container').add(Ext.widget('expenseAccountEditWindow'));
	             win.show();	      
	...
	openCheckWindow:function(grid, rowIndex, colIndex){
		var record = grid.getStore().getAt(rowIndex);
		if (record ) {
			var win = grid.up('container').add(Ext.widget('expenseAccountCheckWindow'));
	             win.show();	      
		      win.down('form').getForm().loadRecord(record);
	      }
	},
	...
	openSearchWindow:function(toolbar, rowIndex, colIndex){
		toolbar.up('grid').up('container').add(Ext.widget('expenseAccountSearchWindow')).show();		
	},
	/*添加记录弹窗按钮*/	
	openAddWindow:function(grid, rowIndex, colIndex){
		grid.up('container').add(Ext.widget('expenseAccountAddWindow')).show();
	},
 /*******************************点击按钮***********************************/
	/*分类单条件查询（快速查询）按钮*/
	quickSearch:function(button){
		...
		Ext.apply(store.proxy.extraParams, {userName:"",timeStart:"",timeEnd:"",checkStatus:""});
  		...
  		if (selModel.hasSelection()) {
    			Ext.Msg.confirm('警告', '确定要删除吗？', function(btn) {
      			if (btn == 'yes') {
        				...
        				Ext.Ajax.request({url:'/expenseaccount/deletes', method:'post', params:{ids:selectIds}, success:function(response, options) {
          					var json = Ext.util.JSON.decode(response.responseText);
          					if (json.status==200) {
            					Ext.Msg.alert('操作成功', json.msg, function() {
            						Ext.data.StoreManager.lookup('expenseAccountStore').load(true);
            					});
         				 	} else {
...
/*******************************提交按钮***********************************/
	...
	editSubmitButton:function(button){
		...
		var  store=Ext.data.StoreManager.lookup('expenseAccountStore');
	    var values=win.down('form').getValues();//获得form数据
	    var record=store.getById(values.id);//获取id获取store中的数据
	    if(record.isValid()){
		record.set(values);
		Ext.Msg.alert('操作成功','修改报销信息成功！');
		Ext.data.StoreManager.lookup('expenseAccountStore').load();
	    }else{
	    	Ext.Msg.alert('操作失败','修改报销信息失败！');
	    }
		win.close();
		//更新事件
	},
	...
	addSubmitButton:function(button){
		...
		var record = Ext.create('Admin.model.expenseaccount.ExpenseAccountModel');
		var values  =form.getValues();//获取form数据 
			console.log(values);
           	record.set(values);
            if(record.isValid()){
           	record.save();
           	Ext.Msg.alert('操作成功','增加报销信息成功！');
           	Ext.data.StoreManager.lookup('expenseAccountStore').load();
            }else{
            	Ext.Msg.alert('操作失败','增加报销信息失败！');
            }
             win.close();
	},
	/*查询多条记录条件提交按钮*/
	...
	checkExpenseAccount:function(button){
		Ext.Msg.confirm('警告', '确定要审核吗？', function(btn) {
  		    ...
  		    var record=store.getById(values.id);//获取id获取store中的数据
		Ext.Ajax.request({
            url: '/expenseaccount/checkstatus',
            method: 'post',
            params: {
            	id:record.id
